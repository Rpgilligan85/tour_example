    ...
    var methods = {
        init: function(opts, startFrom) {
            ...
            if (started == false) {
                ...
                $controlsCss = { 'display': 'none', 'position': 'fixed', 'width': '200px', 'padding': '10px 20px', 'border-radius': '10px', 'font-family': 'sans-serif', 'z-index': '1000' };
        ...
        setTooltip: function(step, stepData, dir) {
            if (options.useOverlay) {
                if (!$overlay) {
                    $overlay = $('<div id="touroverlay" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: '+options.overlayZindex+'; background-color: rgba(0,0,0,0.8);">');
                }
                $('body').append($overlay);
            }
